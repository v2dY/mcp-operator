...
docker-kind-load: ## Load docker image with the manager.
...
# Добавить в самый конец Makefile (после последней строки):

##@ Local Testing

.PHONY: test-operator
test-operator: ## Quick operator testing cycle
	@echo "🚀 Testing operator logic locally..."
	
	# 1. Build operator image
	$(MAKE) docker-build IMG=mcp-operator:dev
	
	# 2. Load into kind cluster
	kind load docker-image mcp-operator:dev
	
	# 3. Generate helm with local settings
	$(MAKE) helm
	@sed -i 's|ghcr.io/v2dy/kmcp|mcp-operator|' helm/values.yaml
	@sed -i 's|VERSION_PLACEHOLDER|dev|' helm/values.yaml
	@sed -i 's|pullPolicy: IfNotPresent|pullPolicy: Never|' helm/values.yaml
	
	# 4. Deploy operator
	@helm upgrade --install mcp-operator-dev helm/ -n operator-system-dev --create-namespace
	
	@echo "✅ Operator deployed!"
	@echo "📋 Next steps:"
	@echo "   make operator-logs"
	@echo "   kubectl apply -f examples/mcpserver-sample.yaml"

.PHONY: operator-logs
operator-logs: ## Show operator logs
	kubectl logs -f deployment/mcp-operator -n operator-system-dev

.PHONY: operator-status
operator-status: ## Check operator status
	@echo "🔍 Operator status:"
	@kubectl get pods -n operator-system-dev
	@echo ""
	@echo "🔍 CRDs:"
